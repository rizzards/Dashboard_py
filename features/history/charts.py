...
def create_income_diff_chart(df, income_col, selected_type):
    """
    Create a quarterly income difference chart showing quarter-over-quarter changes.
    ...
    if not df.empty:
        ...
        if len(quarterly_data) > 1:
            ...
            colors = ['green' if val > 0 else 'red' if val < 0 else 'gray'
                     for val in quarterly_data['income_diff'].fillna(0)]
